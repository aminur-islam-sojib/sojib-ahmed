...
export default function DownloadButton({ disabled = false, cvUrl = "/Sojib_Ahmed_Resume.pdf" }: SentButtonProps) {
  return (
    <div className=" flex w-full md:justify-end my-5">
      <a
        ...
        target="_blank"
        type="submit"
        download="Sojib_Ahmed_Resume.pdf"
      >
        ...
        <span className="flex gap-2 justify-center items-center h-full font-semibold">
          <Send
            size={18}
            className="transition-transform group-hover:scale-110"
          />
          {disabled ? "Downloading..." : "Download Cv"}
        </span>
      </a>
    </div>
  );
